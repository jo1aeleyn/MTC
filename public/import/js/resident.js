            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function createChildInputs(NumberOfDependent) {
                    dependentContainer.innerHTML = ''; // Clear previous inputs
                        ...
                        <div class="card mt-3">
                            ...
                            <div class="row mt-2">
                                <div class="mb-3 col-md-3">
                                    ...
                                    <div class="d-flex flex-column">
                                        <div class="btn-group mb-2" role="group" aria-label="Blu Card toggle button group">
                                ...
                                <div class="mb-3 col-md-3">
                                    <label for="solo_parent_yes" class="form-label">Solo Parent:</label>
                                    <div class="d-flex flex-column">
                                        <div class="btn-group mb-2" role="group" aria-label="Solo Parent toggle button group">
                                            <input type="radio" class="btn-check" name="dependent[${i}][solo_parent]" id="solo_parent_yes_${i}" value="Yes" required>
                                            <label class="btn btn-outline-primary" for="solo_parent_yes_${i}">Yes</label>
                                            <input type="radio" class="btn-check" name="dependent[${i}][solo_parent]" id="solo_parent_no_${i}" value="No" required>
                                            <label class="btn btn-outline-primary" for="solo_parent_no_${i}">No</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                NumberOfDependentInput.addEventListener('input', function () {
                    const NumberOfDependent = parseInt(this.value, 10);
                    if (NumberOfDependent > 0) {
                        createChildInputs(NumberOfDependent);
                        for (let i = 0; i < NumberOfDependent; i++) {
                            updateStudentDetails(i);
                        }
                    } else {
                        dependentContainer.innerHTML = ''; // Clear all inputs if 0 or less
                    }
                });
            
                // Initialize with existing data if any (for edit view)
                if (NumberOfDependentInput.value > 0) {
                    createChildInputs(NumberOfDependentInput.value);
                    for (let i = 0; i < NumberOfDependentInput.value; i++) {
                        updateStudentDetails(i);
                    }
                }
            });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            householdCodeSelect.addEventListener('change', function() {
                ...
                if (selectedUniqueCode) {
                    uniqueCodeInput.value = selectedUniqueCode; // Set the selected unique code to the input field
                }
            });
        
            // Initial population of the dropdown if house number and street name are pre-filled
            if (houseNumberInput.value.trim() && streetNameSelect.value.trim()) {
                updateHouseholdCodes();
            }

            function updateHiddenInput() {
                var uniqueCodeInput = document.getElementById('unique_code');
                console.log(uniqueCodeInput); // This should log the element or null if not found
            
                if (uniqueCodeInput) {
                    uniqueCodeInput.value = 'new value'; // Replace 'new value' with the actual value
                } else {
                    console.error('Element with ID "unique_code" not found.');
                }
            }
